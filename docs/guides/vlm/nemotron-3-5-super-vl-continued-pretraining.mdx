...
the BF16 checkpoint
`nvidia/NVIDIA-Nemotron-3.5-Super-VL-120B-A12B-BF16`. The complete
...
- **Storage**: Approximately 232 GiB for the BF16 checkpoint, and approximately 6 TiB of additional shared storage for checkpoint retention
...
if the model requires access. Download the BF16 model once into the shared cache before starting a
...
The BF16 checkpoint is approximately 232 GiB (249 GB). A full sharded training
